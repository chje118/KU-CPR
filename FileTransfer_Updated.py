...
def merge_and_move_folders(src_folder, dest_folder):
    ...
    try:
        ...
        for src_dir, subdirs, files in os.walk(src_folder):
            ...
            for file in tqdm(files, desc=f"Moving files in {src_dir}", leave=False):
        ...
        print(f"Process interuptedby user.")
...
def transfer_folder_with_retry(source_folder, destination_folder, max_retries = 10, retry_delay = 5):
    ...
    while retry_count <= max_retries:
        try:
            ...
            # Merge folder if the folder already exists
            if new_destination.exists():
                ...
                if not merge_and_move_folders(source_folder, new_destination):
                    print(f"Transfer failed. Error while merging folders.")
                    break
            else:
                shutil.move(source_folder, new_destination)
                print(f"Successfully moved: {source_folder} to {new_destination}")
            ...
            if "network error" not in e: 
               ...
               retry_count += 1
               print(f"Network error occurred. Retry: {retry_count}/{max_retries}")
        ...
        with tqdm(total=len(source_folders), desc="Transferring Folders", unit="folder") as folder_pbar:
            ...
            for source_folder in source_folders:
                ...
                if not os.path.exists(source_folder):
                    ...
                    folder_pbar.update(1)  # Increase progress bar by 1
                    continue
